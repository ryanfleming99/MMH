        ...
        >
          {filtered.map(post => (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              initial={{ opacity: 0 }}
              exit={{ opacity: 0 }} // Corrected typo
              key={post.id}
            >
              <div
                className="relative"
                style={{ backgroundImage: `url(${post.image})` }} // Corrected syntax
              >
                <Link href={`/posts/${post.id}`}>
                  <Image
                    alt={`Thumbnail image for ${post.title}`}
                    className="w-full h-52 mx-auto object-cover"
                    src={post.thumbnailImage || "/default-thumbnail.jpg"} // Fallback image
                  />
